...
import torch
import torch.nn.modules.transformer as tr
...
class MyTransformerEncoder(TransformerEncoder):
    ...
    def forward(self, src: Tensor, mask: Optional[Tensor] = None, src_key_padding_mask: Optional[Tensor] = None, prompt_embeds: Optional[Tensor] = None) -> Tensor:
...
class MyPreNormEncoderLayer(nn.TransformerEncoderLayer):
    ...
    def __init__(self, d_model: int, nhead: int, dim_feedforward: int = 2048, dropout: float = 0.1,
        ...
        if isinstance(activation, str):
            activation = tr._get_activation_fn(activation)

        # We can't test self.activation in forward() in TorchScript,
        # so stash some information about it instead.
        if activation is nn.functional.relu:
            self.activation_relu_or_gelu = 1
        elif activation is nn.functional.gelu:
            self.activation_relu_or_gelu = 2
        else:
            self.activation_relu_or_gelu = 0
        self.activation = activation
    
    def forward(self, src, src_mask=None, src_key_padding_mask=None, prompt_embeds=None):
        # Self attention block
        att = self.norm1(src)
        att = self.self_attn(att, att, att, attn_mask=src_mask, key_padding_mask=src_key_padding_mask, prompt_embeds=prompt_embeds)[0]
...
class MyMultiheadAttention(MultiheadAttention):
    def forward(self, query: Tensor, key: Tensor, value: Tensor, key_padding_mask: Optional[Tensor] = None,
                ...
                average_attn_weights: bool = True, prompt_embeds: Optional[Tensor] = None) -> Tuple[Tensor, Optional[Tensor]]:
        ...
        if not self._qkv_same_embed_dim:
            attn_output, attn_output_weights = multi_head_attention_forward(
                ...
                v_proj_weight=self.v_proj_weight, average_attn_weights=average_attn_weights, prompt_embeds=prompt_embeds)
        else:
            attn_output, attn_output_weights = multi_head_attention_forward(
                query, key, value, self.embed_dim, self.num_heads,
                self.in_proj_weight, self.in_proj_bias,
                ...
                training=self.training,
                key_padding_mask=key_padding_mask, need_weights=need_weights,
                attn_mask=attn_mask, average_attn_weights=average_attn_weights, prompt_embeds=prompt_embeds)
        if self.batch_first and is_batched:
...
def multi_head_attention_forward(
    ...
    if prompt_embeds is not None:
        k = torch.cat([prompt_embeds[0], k], dim=-3)
        v = torch.cat([prompt_embeds[1], v], dim=-3)
