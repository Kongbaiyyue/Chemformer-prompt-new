...
def model_opts(parser):
    ...
    #                "speech-to-text.")
    group.add('--enc_rnn_size', '-enc_rnn_size', type=int, default=256,
              help="Size of encoder rnn hidden states. "
                   "Must be equal to dec_rnn_size except for "
                   "speech-to-text.")
    group.add('--dec_rnn_size', '-dec_rnn_size', type=int, default=256,
              help="Size of decoder rnn hidden states. "
                   "Must be equal to enc_rnn_size except for "
                   "speech-to-text.")
    ...
    group.add('--n_latent', '-n_latent', type=int, default=0,
              help='The number of latent classes to use.')
    group.add('--use_segments', '-use_segments', action='store_true',
              default=False, help='Whether or not to use segment_embeddings')
    # group.add('--use_segments', '-use_segments', action='store_true',
    #           default=True, help='Whether or not to use segment_embeddings')
    group.add('--max_segments', '-max_segments', type=int, default=10,
              help='Maximum number of segments to consider')

    group.add('--max_distance', '-max_distance', nargs='+', default=list(), type=int,
              help='Maximum distance for graph truncated attention')

    group.add('--alpha', '-alpha', default=1.0, type=float,
              help='multiplier of cross_attn loss')
...
def preprocess_opts(parser):
    ...
    group.add('--train_src', '-train_src', nargs='+', default=['data/USPTO-50k_no_rxn/src-train.txt'],
              help="Path(s) to the training source data")
    group.add('--train_tgt', '-train_tgt', nargs='+', default=['data/USPTO-50k_no_rxn/tgt-train.txt'],
    ...
    group.add('--valid_src', '-valid_src', default='data/USPTO-50k_no_rxn/src-val.txt',
              help="Path to the validation source data")
    group.add('--valid_tgt', '-valid_tgt', default='data/USPTO-50k_no_rxn/tgt-val.txt',
              help="Path to the validation target data")

    group.add('--src_dir', '-src_dir', default="",
              help="Source directory for image or audio files.")

    group.add('--save_data', '-save_data', default='data/USPTO-50k_no_rxn/USPTO-50k_no_rxn',
    ...
    group.add('--src_vocab_size', '-src_vocab_size', type=int, default=1000,
              help="Size of the source vocabulary")
    group.add('--tgt_vocab_size', '-tgt_vocab_size', type=int, default=1000,
    ...
    group.add('--add_mask_token', '-add_mask_token', action='store_true',
              help='Adds the mask token to the vocab.')
...
def train_opts(parser):
    ...
    group.add('--data', '-data', default='data/USPTO-50k_no_rxn/USPTO-50k_no_rxn',
              help='Path prefix to the ".train.pt" and '
                   '".valid.pt" file path from preprocess.py')
    ...
    group.add('--valid_steps', '-valid_steps', type=int, default=1,
    ...
    group.add('--max_generator_batches', '-max_generator_batches',
              ...
              help="Maximum batches of words in a sequence to run "
                   "the generator on in parallel. Higher is faster, but "
                   "uses more memory. Set to 0 to disable.")
    group.add('--train_steps', '-train_steps', type=int, default=50000,
    ...
    group.add('--learning_rate', '-learning_rate', type=float, default=0.3,
    ...
    group.add('--reset_cycle_steps', '-reset_cycle_steps', type=int, default=0,
              help='The number of steps to take before resetting the learning')
...
def translate_opts(parser):
    ...
    group.add('--n_translate_latent', '-n_translate_latent', type=int,
              default=1, help='The number of latent classes to use')
    group.add('--n_translate_segments', '-n_translate_segments', type=int,
              default=0, help='If nonzero, the max number of segments')
    ...
    group.add('--output_dir', '-output_dir', default='',
    ...
    group.add('--log_file_level', '-log_file_level', type=str,
              ...
              choices=StoreLoggingLevelAction.CHOICES,
              default="0")
    group.add('--log_probs', '-log_probs', action='store_true', default=False)
