...
def build_pretrain_model_saver(model_opt, opt, model, fields, optim):
    model_saver = PretrainModelSaver(opt.save_model,
                             model,
                             model_opt,
                             fields,
                             optim,
...
class PretrainModelSaver(ModelSaverBase):
    ...
    def save_by_name(self, step, model_name, moving_average=None):
        """Main entry point for model saver
        ...
        chkpt, chkpt_name = self._save(step, save_model, model_name)
        self.last_saved_step = step
    ...
    def _save(self, step, model, model_name):
        real_model = (model.module
                      if isinstance(model, nn.DataParallel)
                      else model)
        ...
        logger.info("Saving checkpoint %s_step_%d.pt" % (self.base_path, step))
        checkpoint_path = '%s_step_%d_%s_g3layer.pt' % (self.base_path, step, model_name)
        torch.save(checkpoint, checkpoint_path)
        return checkpoint, checkpoint_path
