...
class Trainer(object):
    ...
    def validate(self, valid_iter, moving_average=None):
        ...
        valid_model.Bert_encoder.eval()
        valid_model.prompt_encoder.eval()
        valid_model.eval()
        ...
        with torch.no_grad():
            ...
            for batch in valid_iter:
                ...
                if self.n_latent > 1:
                    ...
                    #                                           None, None,
                    #                                           token_embeds=b_emb)
                    outputs, attns = valid_model(src, tgt, src_lengths, segment_input=segment_input, adj=final_adj, prompt_embeds=prompt_embeds)
        ...
        for k, batch in enumerate(true_batches):
            ...
            for j in range(0, target_size-1, trunc_size):
                ...
                if self.n_latent > 1:
                    ...
                    with torch.no_grad():
                        b_emb = self.model.Bert_encoder(src.squeeze(-1), None, src_lengths)
                    # edge_list = [batch.edge for _ in range(3)]
                    # bond_type_list = [batch.bond_type for _ in range(3)]
                    prompt_embeds = self.model.prompt_encoder(batch.atom[0].to(torch.float32), batch.edge[0].to(torch.float32), batch.atom[1][0], n_adj=batch.edge_adj[0], token_embeds=b_emb)
                        # prompt_embeds = self.model.prompt_encoder(None,
                        #                                           None, None,
                        #                                           token_embeds=b_emb)
                    outputs, attns = self.model(src, tgt, src_lengths, bptt=bptt,
                                                segment_input=segment_input, adj=final_adj, prompt_embeds=prompt_embeds)
                bptt = True
