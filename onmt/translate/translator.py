...
class Translator(object):
    ...
    def __init__(
        ...
        self.atom_reader = atom_reader
        self.edge_reader = edge_reader
        self.edge_adj_reader = edge_adj_reader
    ...
    def from_opt(
        ...
        atom_reader = inputters.str2reader["vec"].from_opt(opt)
        edge_reader = inputters.str2reader["vec"].from_opt(opt)
        edge_adj_reader = inputters.str2reader["vec"].from_opt(opt)
        return cls(
            model,
            fields,
            src_reader,
            ...
            atom_reader=atom_reader,
            edge_reader=edge_reader,
            edge_adj_reader=edge_adj_reader,
    ...
    def translate(
        ...
        data = inputters.Dataset(
            self.fields,
            readers=([self.src_reader, self.tgt_reader, self.mask_reader, self.atom_reader, self.edge_reader, self.edge_adj_reader]
                     if tgt else [self.src_reader, self.mask_reader, self.atom_reader, self.edge_reader, self.edge_adj_reader]),
            data=[("src", src), ("tgt", tgt), ("mask", mask), ("atom", atom), ("edge", edge), ("edge_adj", edge_adj)]
                     if tgt else [("src", src), ("mask", mask), ("atom", atom), ("edge", edge), ("edge_adj", edge_adj)],
            dirs=[src_dir, None, None, None, None, None] if tgt else [src_dir, None, None, None, None],
    ...
    def _run_encoder(self, batch):
        ...
        b_emb = self.model.Bert_encoder(src.squeeze(-1), None, src_lengths)
        prompt_embeds = self.model.prompt_encoder(batch.atom[0].to(torch.float32), batch.edge[0].to(torch.float32), batch.atom[1][0], n_adj=batch.edge_adj[0], token_embeds=b_emb)
        # prompt_embeds = self.model.prompt_encoder(None,
        #                                           None, None,
        #                                           token_embeds=b_emb)
        enc_states, memory_bank, src_lengths = self.model.encoder(
            src, src_lengths, prompt_embeds=prompt_embeds, adj=final_adj)
    ...
    def _translate_batch(
        ...
        # 处理预测超出长度的分子
        if not beam.done:
            for i in range(beam.is_finished.size(0)):
                b = beam._batch_offset[i]
                if len(beam.hypotheses[b]) < beam.n_best:
                    best_hyp = sorted(
                        beam.hypotheses[b], key=lambda x: x[0], reverse=True)
                    for n, (score, pred, attn) in enumerate(best_hyp):
                        if n >= beam.n_best:
                            break
                        beam.scores[b].append(score)
                        beam.predictions[b].append(pred)
                        beam.attention[b].append(
                            attn if attn is not None else [])
                    if len(best_hyp) < beam.n_best:
                        for i in range(beam.n_best - len(best_hyp)):
                            beam.scores[b].append(torch.tensor(0).float().to(mb_device))
                            beam.predictions[b].append(torch.tensor([2]).to(mb_device))
                            beam.attention[b].append(torch.tensor([]))
